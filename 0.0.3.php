...
function generate_uid() {
    ...
    } while (file_exists(get_user_file($uid)));
    return $uid;
}
...
function generate_captcha($type = 'login') {
    $ops = ['+', '-', '*'];
    ...
    $question = "{$a} {$op} {$b} = ?";
    $_SESSION['captcha_answer_' . $type] = (string)$ans;
    $_SESSION['captcha_time_' . $type] = time();
    return $question;
}

function verify_captcha($answer, $type = 'login') {
    $key = 'captcha_answer_' . $type;
    $timeKey = 'captcha_time_' . $type;
    if (empty($_SESSION[$key]) || empty($_SESSION[$timeKey])) {
        return false;
    }
    ...
    $ok = trim($answer) === (string)$_SESSION[$key];
    unset($_SESSION[$key], $_SESSION[$timeKey]);
    return $ok;
}
...
function get_user_file($uid) {
    return USERS_DIR . '/' . sanitize($uid) . '.json';
}
...
switch ($action) {
    ...
    case 'getCaptcha':
        $captchaType = sanitize($_GET['type'] ?? 'login');
        if (!in_array($captchaType, ['login', 'register'])) $captchaType = 'login';
        success(['question' => generate_captcha($captchaType)]);
        break;
    ...
    case 'register':
        ...
        if (!empty($settings['require_captcha_register'])) {
            ...
            if (!verify_captcha($captchaAnswer, 'register')) {
        ...
        $result = with_file_lock(DATA_DIR . '/.register.lock', function() use ($usernameInput, $password) {
            ...
            $settings = json_read(DATA_DIR . '/settings.json', []);
            $maxUsers = isset($settings['max_users']) ? intval($settings['max_users']) : 0;
            if ($maxUsers > 0) {
                $currentCount = count(glob(USERS_DIR . '/*.json'));
                if ($currentCount >= $maxUsers) {
                    return ['error' => '注册人数已达上限，请联系管理员'];
                }
            }
    ...
    case 'login':
        ...
        if (!empty($settings['require_captcha_login'])) {
            $captchaAnswer = $_POST['captcha_answer'] ?? '';
            if (!verify_captcha($captchaAnswer, 'login')) {
                error('人机验证答案错误，请重新计算');
            }
        }
    ...
    case 'adminUsers':
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if ($adminAction === 'delete') {
                ...
                $files = glob(USERS_DIR . '/*.json');
                foreach ($files as $f) {
                    $u = get_user_profile(basename($f, '.json'));
                    if ($u) $allUsers[] = $u;
                }
    ...
    case 'adminSettings':
        ...
        if (isset($_POST['max_users'])) {
            $settings['max_users'] = max(0, intval($_POST['max_users']));
        }
    ...
    <div id="adminSection" class="hidden">
        ...
        <div id="tab-skin" class="tab-content hidden">
            ...
            <div class="card">
                ...
                <div class="form-group">
                    <label>皮肤模型</label>
                    <select id="skinModel">
                        <option value="steve">标准</option>
                        <option value="alex">纤细</option>
                    </select>
                </div>
        ...
        <div id="tab-settings" class="tab-content hidden">
            ...
            <div class="card">
                ...
                <div class="form-group">
                    <label>允许注册的总用户上限（0=不限制，建议小于100）</label>
                    <input type="number" id="settingMaxUsers" min="0" max="9999" placeholder="50">
                </div>
            ...
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <div class="text-sm text-muted mt-2">
                        Offline: 采用离线算法，确保兼容性<br>
                        Hash: 基于UID生成，更稳定可靠<br><br>
...
<div class="modal-overlay" id="captchaModal">
    <div class="modal">
        <div class="modal-header"><h3 id="captchaModalTitle">安全验证</h3></div>
        <div class="modal-body">
            <p class="text-sm text-muted mb-4">请完成以下算术题以继续操作</p>
            <div class="form-group">
                <label>题目 <span class="captcha-question" id="modalCaptchaQuestion"></span></label>
                <input type="text" id="modalCaptchaAnswer" placeholder="输入计算结果" autocomplete="off" autofocus>
            </div>
            <div class="captcha-hint">请通过人机验证以进行下一步，确保您不是伪人</div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-block" id="confirmCaptchaBtn">确认</button>
        </div>
    </div>
</div>
...
async function loadCaptcha(type) {
    var data = await apiCall("getCaptcha", { type: type });
    if (data.success && data.question) {
        var questionEl = document.getElementById("modalCaptchaQuestion");
        var inputEl = document.getElementById("modalCaptchaAnswer");
...
async function handleLogin(e) {
    ...
    var s = await apiCall("getSettings");
    if (s.success && s.require_captcha_login) {
        showCaptchaModal("login", function(captchaAnswer) {
            doLogin(username, password, captchaAnswer);
        });
    } else {
...
async function doLogin(username, password, captchaAnswer) {
    var params = { username: username, password: password };
    if (captchaAnswer) params.captcha_answer = captchaAnswer;
...
async function handleRegister(e) {
    ...
    var s = await apiCall("getSettings");
    if (s.success && s.require_captcha_register) {
        showCaptchaModal("register", function(captchaAnswer) {
            doRegister(username, password, captchaAnswer);
        });
    } else {
...
async function doRegister(username, password, captchaAnswer) {
    var params = { username: username, password: password };
    if (captchaAnswer) params.captcha_answer = captchaAnswer;
...
async function loadUsers() {
    ...
    data.users.forEach(function(u) {
        ...
        html += "<tr>" +
            ...
            "<td>" +
                "<button class=\"btn btn-sm btn-secondary\" onclick=\"openRoleModal(\'" + escapeHtml(u.uid) + "\', \'" + u.role + "\', \'" + escapeHtml(u.username) + "\')\">改角色</button> " +
                "<button class=\"btn btn-sm btn-danger\" onclick=\"confirmDeleteUser(\'" + escapeHtml(u.uid) + "\', \'" + escapeHtml(u.username) + "\')\">删除</button>" +
            "</td>" +
        "</tr>";
    });
...
async function loadSettings() {
    ...
    if (data.success) {
        ...
        document.getElementById("settingMaxUsers").value = (data.max_users === undefined || data.max_users === null) ? 50 : data.max_users;
...
async function saveRegisterSettings() {
    var data = await apiCall("adminSettings", {
        ...
        max_users: document.getElementById("settingMaxUsers").value,
...
var captchaModalType = "";

function showCaptchaModal(type, callback) {
    captchaCallback = callback;
    captchaModalType = type;
    document.getElementById("captchaModalTitle").textContent = type === "register" ? "注册验证" : "登录验证";
    document.getElementById("modalCaptchaAnswer").value = "";
    document.getElementById("captchaModal").classList.add("active");
    loadCaptcha(type);
    setTimeout(function() {
        document.getElementById("modalCaptchaAnswer").focus();
    }, 100);
}
...
document.getElementById("confirmCaptchaBtn").addEventListener("click", function() {
    var answer = document.getElementById("modalCaptchaAnswer").value.trim();
    if (!answer) {
        showToast("请输入答案", "error");
        return;
    }
    if (captchaCallback) {
        captchaCallback(answer);
        closeCaptchaModal();
    }
});

document.getElementById("modalCaptchaAnswer").addEventListener("keydown", function(e) {
    if (e.key === "Enter") {
        document.getElementById("confirmCaptchaBtn").click();
    }
});
